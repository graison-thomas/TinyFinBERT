...
## Thesis Overview
Welcome to the repository for "Enhancing TinyFinBERT for Financial Sentiment Analysis Using GPT-Augmented FinBERT Distillation." This thesis explores the use of large language models (LLMs) like GPT-4 Omni to generate synthetic, domain-specific training data, enhancing transformer-based models for financial sentiment analysis. For a detailed overview, please refer to the [thesis](https://arxiv.org/abs/2409.18999).

## Key Objectives
- **Enhance [FinBERT](https://huggingface.co/ProsusAI/finbert) with Synthetic Data**: Utilize data augmented by [GPT-4 Omni](https://openai.com/index/hello-gpt-4o/) to improve FinBERT's learning.
- **Distill Knowledge to TinyFinBERT**: Implement knowledge distillation from augmented FinBERT to TinyFinBERT using both labelled and unlabelled data generated by [GPT-3.5 Turbo](https://platform.openai.com/docs/models/gpt-3-5-turbo) and GPT-4 Omni.
- **Evaluate Model Performance**: Compare the performance of TinyFinBERT against FinBERT and Augmented FinBERT across various financial datasets.
- **Test Generalization**: Assess TinyFinBERT’s ability to generalize on unseen financial data.
...
## Datasets
We employed several datasets to train and evaluate TinyFinBERT effectively:
